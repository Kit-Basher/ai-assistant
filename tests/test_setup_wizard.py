...
class TestSetupWizard(unittest.TestCase):
    ...
    def test_run_setup_wizard_api_down(self) -> None:
        ...
        self.assertIn("personal-agent-api", result.next_action)
    ...
    def test_run_setup_wizard_uses_telegram_runtime_state_when_enabled_but_stopped(self) -> None:
        ...
        self.assertNotEqual("TELEGRAM_DOWN", result.recovery_mode)
        self.assertEqual("No action needed.", result.next_action)

    def test_run_setup_wizard_required_telegram_runtime_state_when_stopped(self) -> None:
        def _fetch(**kwargs: object) -> tuple[bool, dict[str, object] | str]:
            path = str(kwargs.get("path") or "")
            if path == "/ready":
                return True, {
        ...
        with patch.dict(os.environ, {"TELEGRAM_REQUIRED": "1"}, clear=False):
            with patch(
                "agent.setup_wizard.get_telegram_runtime_state",
                return_value={
                    "enabled": True,
