...
class TestReleaseBundle(unittest.TestCase):
    ...
    def test_build_release_bundle_creates_archive_and_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((bundle_dir / "payload" / "agent" / "BUILD_INFO.json").is_file())
            ...
            self.assertTrue(
                (bundle_dir / "payload" / "docs" / "operator" / "INTERNAL_WRITER_REGISTRY_V1.json").is_file()
            )
            ...
            self.assertIn("agent/BUILD_INFO.json", manifest["payload"])
            self.assertIn("docs/operator/INTERNAL_WRITER_REGISTRY_V1.json", manifest["payload"])
            self.assertRegex(str(manifest.get("git_commit") or ""), r"^[0-9a-f]{7,}$")
            self.assertTrue(str(manifest.get("build_time") or "").endswith("Z"))
            build_info = json.loads((bundle_dir / "payload" / "agent" / "BUILD_INFO.json").read_text(encoding="utf-8"))
            self.assertEqual(manifest["git_commit"], build_info["git_commit"])
            self.assertEqual(manifest["build_time"], build_info["build_time"])
