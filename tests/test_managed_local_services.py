...
class TestManagedLocalServicesEndpointAndChat(unittest.TestCase):
    ...
    def test_setup_prompt_with_podman_available_shows_preview(self) -> None:
        ...
        self.assertIn("Creates: the managed local search service.", text)
        self.assertIn("Changes: the local search setting, the managed search configuration.", text)
        self.assertIn("Deletes: nothing.", text)
        self.assertIn("Rollback limit:", text)
        self.assertNotIn("Executor ID:", text)
    ...
    def test_telegram_search_setup_plan_text_is_readable_and_redacted(self) -> None:
        ...
        self.assertIn("Review this change before approving:", rendered)
        self.assertIn("Creates: the managed local search service.", rendered)
        self.assertNotIn("Executor ID:", rendered)
