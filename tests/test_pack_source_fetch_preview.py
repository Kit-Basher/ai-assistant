...
class TestPackSourceFetchPreview(unittest.TestCase):
    ...
    def test_approved_archive_previews_are_quarantine_only(self) -> None:
        for url, kind in (
            ...
            with self.subTest(kind=kind):
                ...
                self.assertIn("quarantine", preview.user_message.lower())

    def test_fetch_import_result_advances_only_to_review(self) -> None:
    ...
    def test_blocked_archive_hardening_still_blocks_malicious_archive(self) -> None:
        ...
        self.assertFalse(result.did_grant_permissions)
        self.assertFalse(result.did_use_pack)
        self.assertEqual("traversal_entries_rejected", result.blocked_reason)
    ...
    def test_repeated_yes_equivalent_does_not_skip_review_approval(self) -> None:
        ...
        self.assertTrue(first.imported_for_review)
        self.assertTrue(second.imported_for_review)
        self.assertTrue(all(not row.get("approved") and not row.get("enabled") for row in self.store.list_external_packs()))
