...
- Remaining managed-action reliability gaps are audited. Highest priority next target is pack removal/source deletion cleanup; semantic-memory indexing/scheduled observe writes, bulk preference resets, notification send/action-ledger appends, support bundle artifacts, package install/directory creation shell flows, and future filesystem writes remain tracked follow-ups.
