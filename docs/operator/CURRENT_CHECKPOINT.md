...
- Confirm-gated SearXNG managed local service setup can run only the approved Docker/Podman image, name, loopback bind, and approved primary/fallback ports. The first managed install uses the image default config and no `/etc/searxng` bind mount.
