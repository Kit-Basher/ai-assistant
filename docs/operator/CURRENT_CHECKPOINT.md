...
- Remaining managed-action reliability gaps are audited. Highest priority next target is semantic-memory indexing/scheduled observe writes if they become normal user-triggered managed actions; bulk preference resets, package install/directory creation shell flows, and future filesystem writes remain tracked follow-ups. Remote notification delivery and action ledger records remain append-only by design after local readback verification.
