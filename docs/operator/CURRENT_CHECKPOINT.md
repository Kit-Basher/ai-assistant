...
- Remaining managed-action reliability gaps are audited. Highest priority next target is memory/bootstrap and onboarding/preferences writes; pack removal/source deletion, notification send/action-ledger appends, support bundle artifacts, package install/directory creation shell flows, and future filesystem writes remain tracked follow-ups.
