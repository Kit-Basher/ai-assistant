...
| `scripts/rc1_latency_closure_smoke.py` | RC1 latency distribution proof for `/ready`, `/state`, `/search/status`, package-state lookup, package preview, and pending-action status. | Medium | Yes | No | No | Yes for final v0.2.1 latency closure | Yes | Keep while closing v0.2.1; later fold into perf smoke if stable. |
