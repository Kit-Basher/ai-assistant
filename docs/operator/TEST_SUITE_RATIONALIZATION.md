...
| `scripts/capability_policy_audit.py` | Capability registry and executor-binding audit with explicit warnings for documented legacy/unmigrated paths. | Fast | No | No | Yes | Yes for capability policy changes; documented unmigrated warnings are expected | No | Keep; warnings should shrink only through real executor migration. |
